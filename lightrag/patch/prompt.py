# PATCH LOCAL (bind-mounted por cima de /app/lightrag/prompt.py — ver docker-compose.yml).
# Base: imagem ghcr.io/hkuds/lightrag@sha256:49eacfa9... (extraida com `docker exec fai_lightrag
# cat /app/lightrag/prompt.py`). Mudancas EXCLUSIVAS nos templates "rag_response" e
# "naive_rag_response" (itens 1/2/4/5/6 das Instructions): o protocolo nativo de citacao
# ("### References" com "[n] Titulo") CONFLITAVA com o protocolo do nosso user_prompt
# ("> Fonte: [arquivo.pdf, pags. N]"), fazendo o modelo ora seguir um, ora outro (painel de
# fontes vazio); e a negativa seca ("not enough information") conflitava com o Protocolo de
# Negativa da Lina. O patch alinha o template ao nosso protocolo e declara o user_prompt
# AUTORITATIVO. Ao atualizar a imagem: re-extrair o prompt.py, reaplicar estas mudancas e
# atualizar o digest fixado no compose.
...
1. Step-by-Step Instruction:
  ...
  - Each page of the Document Chunks starts with a `[PÁGINA N]` marker: all content after that marker (until the next one) belongs to page N. Track the pages of the chunks which directly support the facts presented in the response. NEVER reproduce the `[PÁGINA N]` token in the response.
  - If any sentence of the response uses information from the Document Chunks, end the response with ONE final source line (see item 4). Do NOT generate a `### References` section.
  - Do not generate anything after the source line.

2. Content & Grounding:
  - Strictly adhere to the provided context from the **Context**; DO NOT invent, assume, or infer any information not explicitly stated.
  - If the answer cannot be found in the **Context**, do not attempt to guess. Follow the negative-answer protocol defined in the Additional Instructions below; NEVER reply with a bare "I do not have enough information".
...
4. Source Line Format:
  - ONE single line at the very end of the response, on its own line, never inside a table: `> Fonte: [File_Name.pdf, págs. N, M]` (use `pág. N` for a single page).
  - List all used pages in ascending order, without repetition. Page numbers are REQUIRED whenever `[PÁGINA N]` markers exist in the chunks used.
  - Use the file name EXACTLY as it appears in the `Reference Document List` (original language, no abbreviation).
  - Omit the source line ONLY when no sentence of the response came from the Document Chunks.

5. Source Line Example:
```
> Fonte: [Manual dos Coordenadores.pdf, págs. 14, 15]
```

6. Additional Instructions (AUTHORITATIVE: in any conflict with the instructions above, the Additional Instructions below PREVAIL): {user_prompt}
