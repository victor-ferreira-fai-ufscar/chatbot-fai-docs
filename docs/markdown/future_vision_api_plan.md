# Planos Futuros: Integração da Google Cloud Vision API (Híbrido)
...
## 2. Leitura de Prints de Telas e Sistemas Para dúvidas como: *"Onde clico para lançar minha nota no sistema acadêmico?"* ou *"Qual a estrutura de pastas do RH?"*.
...
## 3. Estratégia Principal a ser Aplicada: O Funil Híbrido Por tratar-se de uma API paga pela quantidade de imagens e uso, não passaremos massivamente e às cegas todos os PDFs na plataforma do Google Cloud, pois causará impacto financeiro indesejado a cada reindexação.
